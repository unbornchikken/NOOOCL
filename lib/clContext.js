...
function CLContext(devices, properties) {
    ...
    var handle = this.cl.createContext(bridjs.byPointer(propArray), deviceCount, bridjs.byPointer(deviceArray), null, null, bridjs.byPointer(err));
...
function CLContext(platform, deviceType, properties) {
    deviceType = deviceType || cl.DEVICE_TYPE_ALL;

    var propArray = createPropArray(properties, firstDevice.platform);

    var err = bridjs.newNativeValue(nodecl.Type.errcode);
    var handle = this.cl.createContext(bridjs.byPointer(propArray), deviceType, null, null, bridjs.byPointer(err));
